  ...
  if (queryText && typeof queryText === "string" && queryText.trim()) {
    ...
    filtered = filtered
      .map((k) => {
        const text = `${k.title}\n${k.body}`.toLowerCase();
        const score =
          (q && text.includes(q) ? 1 : 0) +
          (k._categories || []).reduce(
            (acc, c) => acc + (q.includes(String(c).toLowerCase()) ? 0.5 : 0),
            0
          );
...
function buildKnowledgePrimer(knowledge, categories) {
  ...
  const lines = limited.map((k, idx) => {
    ...
    return `- (${idx + 1}) ${k.title}: ${String(k.body).slice(
      0,
      500
    )}\n  tags: ${tags}`;
  });
...
export async function messageAI(req, res, next) {
  try {
    ...
    const finalCategories = Array.from(
      new Set([...(autoCategories || []), ...provided])
    );

    // Fetch knowledge and context
    const knowledge = await getKnowledgeForConversation(
...
function inferConversationMode(style, categories) {
  ...
  if (haystack.includes("advice") || haystack.includes("resource"))
    return "advice";
  if (
